...
public class OperationHistoryDaoImpl implements OperationHistoryDao {
    private final static String SELECT_OPERATION_HISTORY_BY_USER="select *\n" +
    ...
    private final static String SELECT_COUNT_OPERATION_FOR_USER="SELECT COUNT(ID) FROM OPERATIONS_HISTORY " +
            "WHERE ORDER_ID IN (SELECT ID FROM ORDERS WHERE USER_ID=:userId)";
